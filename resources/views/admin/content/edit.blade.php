            ...
            <x-admin.card :title="$g['label']">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    @foreach ($g['fields'] as $f)
                        @php $name = "{$group}[{$f['key']}]"; $val = $values[$group][$f['key']] ?? ''; $wide = in_array($f['type'], ['textarea', 'repeater', 'images']); @endphp
                        ...
                        @elseif ($f['type'] === 'images')
                            <div class="md:col-span-2" x-data="{ items: @js(array_values($val ?: [])), files: [], pick(e) { this.files = [...e.target.files].map(f => ({ name: f.name, url: URL.createObjectURL(f) })); } }">
                                <div class="mb-2 flex items-center justify-between">
                                    ...
                                    <label class="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-brand-border bg-white px-3 py-2 text-label font-medium hover:bg-brand-light"><input type="file" name="{{ $name }}_files[]" accept="image/*" multiple class="sr-only" @change="pick"><x-heroicon-o-arrow-up-tray class="h-4 w-4" />Unggah logo</label>
                                </div>
                                <div class="grid grid-cols-2 gap-2 md:grid-cols-4">
                                    <template x-for="(it, i) in items" :key="it.src">
                                        <div class="rounded-lg border border-brand-border bg-white p-2">
                                            ...
                                            <input type="hidden" :name="`{{ $name }}_keep[${i}][src]`" :value="it.src">
                                            <div class="mt-2 flex items-center gap-1.5">
                                                <input type="text" :name="`{{ $name }}_keep[${i}][caption]`" x-model="it.caption" placeholder="Nama" class="{{ $smallInput }}">
